...
#
# Integer: the highest -log10(p) that will be shown. If no input, will behave normally
...
OUTFILE = args[2]
# y.max = args[3]
y.max <- if (!is.null(args[3]) && !is.na(args[3])) {
    as.integer(args[3])
} else {
    NULL
}
...
# snpsOfInterest <- read.table("snp-highlight.txt", header=F)$V1
...
manhattan(dat[(dat$rsid %in% dat$rsid[match(s, dat$rsid)]) | (dat$p <= p.thresh),],
        chr="chr", bp="bp", p="p", snp="rsid", col=c("gray", "black"),
        logp=TRUE, chrlabs = chrlabs, ylim = c(0,y.max))
}
dev.off()
