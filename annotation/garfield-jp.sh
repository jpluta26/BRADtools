#!/bin/bash
# this is the script included with garfield that runs the full analysis
# this version has the variables filled in for my analysis, and also some minor
# changes to make it run correctly
...
ANNOTDIR=$DATADIR/annotation-custom
...
do
        echo 'CHR'$CHR
        ./garfield-prep-chr -ptags $PRUNETAGSDIR/chr$CHR -ctags $CLUMPTAGSDIR/ch
r$CHR -maftss $MAFTSSDDIR/chr$CHR -pval $PVALDIR/chr$CHR -ann $ANNOTDIR/chr$CHR
-excl 895,975,976,977,978,979,980 -chr $CHR -o $F1 || { echo 'Failure!'; }
done
...
Rscript garfield-test.R -i $F1 -o $F2 -l $ANNOTLINKFILE -pt $PTHRESH -b $BINNING
 -s $SUBSET -c $CONDITION
...
# Rscript garfield-test.R -i $F1 -o $F2 -l $ANNOTLINKFILE -pt $PTHRESH -b $BINNI
NG -s $SUBSET -c $CONDITION -ct $CONDITIONTHRESH -padj $Padj
...
# PENRICH=1e-9 ## Enrichment significance threshold - by default you might want
to use $Padj
...
CLUMPTAGSDIR $ANNOTDIR $PVALDIR $F1 $F2 $GARFIELD_significant_annotations $GARFI
ELD_VARS $PTHRESH $PENRICH
