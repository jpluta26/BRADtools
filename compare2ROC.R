...
#        grp1.name, grp2.name (character), the name of the groups, added to the legend
{
  ...
  text.tcga <- paste(grp1.name, " AUC = ", round(roc.dat1[[2]][[1]][1], 2), sep = "")
  text.pen <- paste(grp2.name, " AUC = ", round( roc.dat2[[2]][[1]][1], 2), sep = "")
  plot(roc.dat1[[1]], col = "blue", main = title)
  plot(roc.dat2[[1]], col = "red", add = TRUE)
  mtext(p.annot, side = 3)
  legend(x = 0.5, y = 0.2, cex = .9, legend = c(text.tcga, text.pen), col = c("blue", "red"), lwd = 3)
  dev.off()
}  
# -------------------------------------------------------------------------- #
