...
# --------------------------- getROC ------------------------------- #
getROC <- function( dat )
  # create ROC curve and get AUTC
  # input: dat, data.frame containing PRS and phenotypes
{
  y <- dat$PHENO
  x <- dat$PRS
  if( class(y) == "numeric" )
  ...
  # create design matrix, including covariate for site
  site.mat <- model.matrix( ~ site - 1, data = cov)
  design.mat <- cbind(x, dat$EV1, dat$EV2, dat$EV3, site.mat)
  out <- glm( y ~ x + dat$EV1 + dat$EV2 + dat$EV3 + dat$site, family = "binomial" )
  
  B <- out$coefficients
  pprob <- as.vector(design.mat %*% B)
  pred <- prediction(pprob, y)
  perf <- performance(pred, 'tpr', 'fpr')
  AUC <- performance(pred, 'auc')@y.values
  roc.out <- roc(response = y, predictor = pprob)
  return(list(roc.out, AUC))
}
# ----------------------------------------------------------------- #
...
fam <- read.table("case_ctl_cauc.fam", header = F, as.is = T)
fam$V2 <- paste(fam$V2, fam$V2, sep = "_")


PRS <- PRS[match(fam$V2, PRS$IID),]
PRS$PHENO <- fam$V6

if( any(PRS$PHENO == -9))
{ stop("missing phenotype") }
...
print(p1)
png("TECAC-PRS.png", height = 400, width = 550)
print(p1)
dev.off()
