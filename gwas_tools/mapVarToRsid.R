...
OUTFILE = args[2]
BUILD = args[3]
dat <- as.data.frame(read.table(SNPLST, header = F))
...
{
	ref <- as.data.frame(fread(paste("/project/knathanslab/REF/HHRC/", BUILD, "/HRC-chrbp-rsid-map-chr", i, ".txt", sep = ""), header = F))
